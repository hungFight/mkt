...
export const RenderInputNumberSanProfile = (data: {
  register: UseFormRegister<FieldValues>
  setFormDT: React.Dispatch<React.SetStateAction<PropsScanProfile>>
  ...
  return (
    <InputNumberField
      ...
      value={type && formDT[key]?.[type] ? formDT[key]?.[type] : formDT[key]}
      clsLabel="whitespace-pre-wrap"
      classInputContainer={`w-full flex items-center justify-start ${classInputContainer}`}
    />
  )
}
